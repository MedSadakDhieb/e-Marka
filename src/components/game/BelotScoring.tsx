...
import { BelotConfig, Round, Team } from '../../types';

export const BelotScoring: React.FC = () => {
  const { gameConfig, rounds, addRound } = useGame();
  const config = gameConfig as BelotConfig;
  ...
  const hasWinner = totalTeam1 >= config.finalScore || totalTeam2 >= config.finalScore;
  
  return (
    <div className="p-4">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-1 text-gray-800 dark:text-gray-100">Belot Scoring</h2>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          First to {config.finalScore} points wins
        </p>
      </div>
      ...
      {hasWinner && (
        <div className="mb-6 p-4 bg-green-100 dark:bg-green-900 rounded-lg text-center">
          <p className="font-bold text-green-800 dark:text-green-200">
            {totalTeam1 >= config.finalScore ? team1.name : team2.name} wins!
          </p>
        </div>
      )}
